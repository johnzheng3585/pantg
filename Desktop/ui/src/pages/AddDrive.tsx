...
export default function AddDrive({ mode = "add" }: AddDriveProps) {
  ...
  const [siteUrl, setSiteUrl] = useState(isReauthorize ? decodedSiteUrl : DEFAULT_API_SITE_URL);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    await startAuthorization(isReauthorize ? siteUrl : DEFAULT_API_SITE_URL);
  };
      ...
      >
        ...
        >
          ...
          {pageState === "success" ? (
            ...
            <>
              <CheckCircleRoundedIcon color="success" sx={{ fontSize: 72, mt: 2 }} />
              <Typography
                sx={{ mt: 2 }}
                variant="h5"
                component="h1"
