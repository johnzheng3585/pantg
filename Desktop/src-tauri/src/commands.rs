...
pub fn show_add_drive_window_impl(app: &AppHandle) {
    show_drive_window_internal(app, "添加糖果盘", &get_url_with_lang("index.html/#/add-drive"));
}
...
pub fn show_reauthorize_window_impl(
    ...
    show_drive_window_internal(app, "重新授权", &get_url_with_lang(&url_path));
}
...
const STARTUP_TASK_ID: &str = "tangguopan";
